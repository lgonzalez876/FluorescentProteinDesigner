...
LINEAGE_URL = "https://www.fpbase.org/ajax/lineage/"
...
def get_ancestors() -> pd.DataFrame:
    """Return DataFrame of root ancestor proteins (name, slug, sequence).

    Fetches lineage data from FPBase, identifies root nodes (direct children
    of "fakeroot"), and looks up their sequences from the full proteins JSON.
    """
    # Fetch lineage data
    lineage_path = CACHE_DIR / "fpbase_lineage.json"
    lineage = _fetch_or_cache(LINEAGE_URL, lineage_path)

    # Find root ancestors: direct children of "fakeroot"
    # Lineage is a tree: {"name": "fakeroot", "children": [...]}
    roots = []
    for node in lineage.get("children", []):
        roots.append({"name": node.get("name", ""), "slug": node.get("slug", "")})

    # Load full proteins JSON for sequence lookup
    proteins_path = CACHE_DIR / "fpbase_proteins.json"
    if not proteins_path.exists():
        download()
    with open(proteins_path) as f:
    ...
    # Match ancestors to sequences
    rows = []
    for root in roots:
        seq = seq_lookup.get(root["slug"], "")
        if seq:
            rows.append({"name": root["name"], "slug": root["slug"], "sequence": seq})

    df = pd.DataFrame(rows)
    print(f"Found {len(df)} ancestor proteins with sequences")
    return df
